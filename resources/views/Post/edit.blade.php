<x-layout :title="$pageTitle">
    ...
    <form method="POST" action="/blogs/{{ $posts->id }}">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-12">
            <div class="border-b border-gray-900/10 pb-12">
                <h2 class="text-base/7 font-semibold text-gray-900">Edit Post {{ $posts->title }}</h2>
                <p class="mt-1 text-sm/6 text-gray-600">This Edit and Publish Post. </p>

                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-3">
                        <label for="title" class="block text-sm/6 font-medium text-gray-900">Title</label>
                        <div class="mt-2">
                            <input id="title" value="{{ old('title', $posts->title) }}" type="text" name="title"
                    ...
                    <div class="sm:col-span-3">
                        ...
                        <div class="mt-2">
                            <input id="author" value="{{old('author', $posts->author) }}" type="text" name="author"
                    ...
                    <div class="col-span-full">
                        <div class="flex gap-3">
                            <div class="flex h-6 shrink-0 items-center">
                                <div class="group grid size-4 grid-cols-1">
                                    <input id="published" type="checkbox" name="published"
                                        value="1" {{ old('published') || (!old() && $posts->published) ? 'checked' : '' }}
                            ...
                            <div class="text-sm/6">
                                <label for="published" class="font-medium text-gray-900">IsPublished ?</label>
                                <p id="published-description" class="text-gray-500">Do You want publish or save in
                                    draft.</p>
                            </div>
                        </div>
                    </div>
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
            <a href="/blogs" class="text-sm/6 font-semibold text-gray-900">Cancel</a>
            <button type="submit"
